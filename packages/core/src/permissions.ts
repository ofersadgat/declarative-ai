/**
 * Tool-call permissions (RUNTIMES-AND-PERMISSIONS.md §4). An agent's tool call is authorized by a MODE —
...
import type { ExecServices } from "./contract";
 ...
 * `full` — every tool; `read-only` — only tools declaring `capabilities.readOnly`; `plan` — read-only until
...
export function isPermissionDenied(v: unknown): v is PermissionDenied {
  return typeof v === "object" && v !== null && (v as PermissionDenied).denied === true;
}
...
export class PermissionLedger {
  ...
  constructor(opts: { baseline?: PermissionBaseline; process?: Map<string, PermissionMode> } = {}) {
    ...
    // Host-owned so an `always` decision survives across runs in the same process (RUNTIMES-AND-PERMISSIONS §4).
...
export function withPermission(
  ...
  return {
    ...
    async run(input: Record<string, unknown>, ctx: ExecServices): Promise<unknown> {
      ...
      if (!inProfile(profile, { name: toolName, readOnly: tool.capabilities?.readOnly === true }, profiles)) {
...
export function planExitTool(opts: { ledger: PermissionLedger; sessionId: string; approve: Approver }): Tool {
  ...
  return {
    ...
    capabilities: { readOnly: true, interactive: true },
