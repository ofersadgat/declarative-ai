...
export function retryAfterMs(err: unknown): number | undefined {
  ...
  // An EMPTY (or whitespace) header is "no signal", not "retry immediately": `Number("")` is a finite
  // 0, which would return a 0 ms backoff and hammer the endpoint instead of falling back to the
  // caller's own backoff. Same for the `Date.parse` path, which would reject it anyway.
...
/**
 * The `cause` descent, capped at the same depth as `classifyError`/`extractRateLimitInfo`. Uncapped it
 * is unbounded recursion: a bare `new Error("")` contributes NO parts (name "Error" and an empty
 * message are both skipped), so a chain that loops back on itself recurses forever and dies with a
 * `RangeError` — on the FAILURE-FORMATTING path, turning a recoverable failure into a crash.
 */
function describeAt(err: unknown, depth: number): string {
  ...
  if (parts.length === 0 && e.cause && depth < 6) parts.push(describeAt(e.cause, depth + 1));
