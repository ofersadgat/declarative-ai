...
describe("SchemaValidator", () => {
  it("validates inline schemas synchronously and caches by content hash", () => {
    ...
    expect(v.validateValue(reordered as Record<string, unknown>, { outcome: "clean" }).ok).toBe(true);
  });
  ...
  it("resolves store-backed $ref graphs through the injected resolver", async () => {
    ...
    const v = new SchemaValidator({
      getSchema: async (id) => (id === "schema:leaf" ? leaf : id === "schema:root" ? root : undefined),
    });
