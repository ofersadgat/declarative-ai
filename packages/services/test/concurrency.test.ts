...
describe("estimateCallTokens", () => {
  it("approximates input (chars/4) and the output ceiling, split", () => {
    expect(estimateCallTokens("x".repeat(40), undefined, 100)).toEqual({ inputTokens: 10, outputTokens: 100 });
    expect(estimateCallTokens("x".repeat(40), "y".repeat(40), 0)).toEqual({ inputTokens: 20, outputTokens: 0 });
    expect(estimateCallTokens("", undefined, undefined)).toEqual({ inputTokens: 0, outputTokens: 512 }); // default output estimate
  });
});
