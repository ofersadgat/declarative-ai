...
import { structuredOutputFromNames } from "../../src/model-catalog.js";
...
describe("profileForCaps — capability-derived structured-output profile", () => {
  it("native claude-* → the SDK-deferring Anthropic profile, regardless of caps", () => {
    ...
    expect(profileForCaps("claude-opus-4-8", structuredOutputFromNames(["max_tokens", "tools", "reasoning"])).id).toBe(ANTHROPIC_AI_SDK.id);
  });

  it("structured_outputs in the names → the `schema` tier → strict json_schema", () => {
    ...
    expect(profileForCaps("qwen/qwen3.7-max", structuredOutputFromNames(["temperature", "response_format", "structured_outputs"])).id).toBe(OPENROUTER_STRICT.id);
    expect(profileForCaps("meta-llama/llama-4-maverick", structuredOutputFromNames(["structured_outputs"])).id).toBe(OPENROUTER_STRICT.id);
    expect(profileForCaps("z-ai/glm-4.7-flash", structuredOutputFromNames(["response_format", "structured_outputs"])).id).toBe(OPENROUTER_STRICT.id);
  });

  it("response_format but no structured_outputs → json_object (object tier)", () => {
    expect(profileForCaps("some/json-object-model", structuredOutputFromNames(["temperature", "response_format"])).id).toBe(JSON_OBJECT.id);
  });

  it("KNOWN caps with neither signal → the text floor (plain-text completion)", () => {
    expect(profileForCaps("some/text-only-model", structuredOutputFromNames(["temperature", "max_tokens"])).id).toBe(ADVISORY.id);
  });
